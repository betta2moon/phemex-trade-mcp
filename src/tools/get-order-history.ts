...
export function registerGetOrderHistory(server: McpServer, client: PhemexClient, productCache: ProductInfoCache) {
  server.tool(
    "get_order_history",
    "Query closed/filled order history for a symbol (USDT-M, Coin-M, or Spot)",
    {
    ...
    async ({ symbol, limit, contractType }) => {
      ...
      const endpoint = ContractRouter.getEndpoint(contractType, "orderHistory");
